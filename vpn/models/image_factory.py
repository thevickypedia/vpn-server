...
def deprecation_warning(image_id: str, deprecation_time: str) -> None:
    ...
    # Start sending warnings 30 days in advance
    if (expired_utc - current_utc).days < 30:
...
class ImageFactory:
    ...
    def get_ami_id_ssm(self) -> str:
        ...
        response = self.ssm_client.get_parameters(
            Names=[ami_base.ALIAS], WithDecryption=True
        )
    ...
    def get_ami_id_product_code(self) -> str:
        ...
        response = self.ec2_client.describe_images(
            Filters=filters, Owners=["aws-marketplace"]
        )
    ...
    def get_ami_id_name(self) -> str:
        ...
        response = self.ec2_client.describe_images(
            Filters=[{"Name": "name", "Values": [ami_base.NAME]}]
        )
        if images := response.get("Images"):
    ...
    def get_image_id(self) -> str:
        ...
        try:
            ...
            if "UnrecognizedClientException" in str(
                error
            ) or "InvalidSignatureException" in str(error):
                raise
