...
def _delete_key_pair(target_key: str = key_name) -> bool:
    try:
        ...
        logger.error(f'API call to delete the key {key_name} has failed.\n{error}')
        return False
...
def _get_public_dns(instance_id: str) -> tuple or None:
...
def configure_openvpn() -> None:
    if (instance_id := create_ec2_instance()) and (tuple_object := _get_public_dns(instance_id=instance_id)):
        public_dns, public_ip = tuple_object
